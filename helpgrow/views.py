...
def helpgrow_page(request):
    ...
    requests = qs.select_related("author").prefetch_related("acceptances", "comments")
...
def comment_request(request, request_id):
    ...
    if form.is_valid():
        ...
        return JsonResponse({"id": c.id, "user": c.user.name, "content": c.content, "created_at": c.created_at.isoformat()})
